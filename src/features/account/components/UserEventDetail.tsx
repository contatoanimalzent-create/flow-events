...
export function UserEventDetail({ event }: UserEventDetailProps) {
  if (!event) {
    return (
      <EmptyState
        title="Escolha um evento para abrir seus acessos"
        description="Assim que voce selecionar um card, mostramos os tickets, o QR code e as instrucoes principais para o dia da experiencia."
      />
    )
  }
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        eyebrow="Seu acesso"
        title={event.name}
        description="Tudo o que voce precisa para chegar ao evento com clareza: tickets, QR codes e instrucoes operacionais."
      />
        ...
        <div className="grid gap-6 rounded-[2.2rem] border border-white/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.92),rgba(249,242,233,0.84))] p-6 shadow-[0_22px_55px_rgba(48,35,18,0.07)] lg:grid-cols-[22rem_minmax(0,1fr)]">
          ...
          <div className="space-y-5">
            <div className="flex flex-wrap gap-2">
              <PremiumBadge tone="accent">{event.category || 'Experiencia premium'}</PremiumBadge>
              <PremiumBadge tone={event.timeframe === 'upcoming' ? 'success' : 'info'}>
                {event.timeframe === 'upcoming' ? 'Proximo evento' : 'Evento passado'}
              </PremiumBadge>
            </div>
            ...
            <div className="grid gap-3 text-sm text-[#5f5549] md:grid-cols-2">
              <div className="flex items-center gap-2 rounded-[1.2rem] border border-[#eee2cf] bg-white/80 px-4 py-3">
                ...
                <span>{new Date(event.startsAt).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</span>
              </div>
              <div className="flex items-center gap-2 rounded-[1.2rem] border border-[#eee2cf] bg-white/80 px-4 py-3">
                <Clock3 className="h-4 w-4 text-[#7b6440]" />
                <span>{new Date(event.startsAt).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</span>
              </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <SurfacePanel className="rounded-[1.35rem] p-4">
                <div className="text-[11px] uppercase tracking-[0.24em] text-[#8e7f68]">Ingressos</div>
                <div className="mt-2 text-2xl font-semibold text-[#1f1a15]">{event.ticketsCount}</div>
              </SurfacePanel>
              <SurfacePanel className="rounded-[1.35rem] p-4">
                <div className="text-[11px] uppercase tracking-[0.24em] text-[#8e7f68]">Ativos</div>
                <div className="mt-2 text-2xl font-semibold text-[#1f1a15]">{event.activeTicketCount}</div>
              </SurfacePanel>
              <SurfacePanel className="rounded-[1.35rem] p-4">
                <div className="text-[11px] uppercase tracking-[0.24em] text-[#8e7f68]">Usados</div>
            ...
            <div className="rounded-[1.5rem] border border-[#eadcc6] bg-[#faf4e8] p-5">
              <div className="text-[11px] uppercase tracking-[0.24em] text-[#8e7f68]">Instrucoes principais</div>
      ...
      {event.tickets.length === 0 ? (
        <EmptyState
          title="Seus tickets ainda estao em emissao"
          description="Assim que o pagamento for confirmado pelo gateway, os ingressos digitais aparecem aqui automaticamente."
        />
      ) : (
        <div className="space-y-5">
