...
export function buildConsistencyIssues(params: BuildConsistencyIssuesParams): {
  ...
  for (const ticket of params.digitalTickets) {
    ...
    issues.push({
      ...
      description: `Ticket ${ticket.id.slice(0, 8).toUpperCase()} nao aponta para um order_item existente.`,
  ...
  for (const run of params.campaignRuns) {
    ...
    issues.push({
      ...
      severity: 'warning',
      status: 'open',
      title: 'Execucao de campanha sem destinatarios',
      description: `Run ${run.name} foi criada com audiencia > 0 mas ainda nao possui recipients.`,
