...
const FEATURES = [
  ...
  { icon: Users, title: 'Gestão completa de staff', desc: 'Ponto digital via QR, escala, supervisão, relatórios. Tudo que o Weezevent tem, com metade da complexidade.', tag: 'EQUIPE' },
    ...
    <div className="bg-[#080808] text-[#f5f5f0] overflow-x-hidden">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-8 py-5"
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[10px] text-[#6b6b6b] font-mono tracking-widest uppercase border border-[#242424] px-2 py-0.5 rounded-sm">Events</span>
        </div>
      ...
      <section ref={heroRef} className="relative min-h-screen flex flex-col items-center justify-center overflow-hidden py-32">
        <div className="absolute inset-0 opacity-[0.04]"
          style={{ backgroundImage: 'linear-gradient(#d4ff00 1px, transparent 1px), linear-gradient(90deg, #d4ff00 1px, transparent 1px)', backgroundSize: '80px 80px', transform: `translate(${mousePos.x * 0.3}px, ${mousePos.y * 0.3}px)`, transition: 'transform 0.1s ease-out' }} />
        <div className="absolute w-[700px] h-[700px] rounded-full pointer-events-none"
          style={{ background: 'radial-gradient(circle, rgba(212,255,0,0.07) 0%, transparent 70%)', top: '50%', left: '50%', transform: `translate(-50%, -50%) translate(${mousePos.x * 0.8}px, ${mousePos.y * 0.8}px)`, transition: 'transform 0.15s ease-out' }} />
        <div className="absolute w-[400px] h-[400px] rounded-full pointer-events-none"
          style={{ background: 'radial-gradient(circle, rgba(212,255,0,0.04) 0%, transparent 70%)', bottom: '15%', right: '10%', transform: `translate(${-mousePos.x * 0.5}px, ${-mousePos.y * 0.5}px)`, transition: 'transform 0.2s ease-out' }} />
        ...
        <div className="relative z-10 flex items-center gap-2 mb-8 animate-fade-in">
          ...
          <span className="text-[11px] font-mono tracking-[0.3em] text-[#d4ff00] uppercase">Animalz Group · Plataforma de Eventos</span>
        </div>

        <h1 className="relative z-10 text-center leading-none animate-slide-up"
          style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 'clamp(64px, 12vw, 160px)', letterSpacing: '-0.02em', transform: `translateY(${scrollY * 0.15}px)` }}>
        ...
        <div className="flex gap-16 animate-marquee whitespace-nowrap" style={{ width: 'max-content' }}>
          {[...MARQUEE_ITEMS, ...MARQUEE_ITEMS].map((item, i) => (
            <span key={i} className="flex items-center gap-4 text-xs font-mono tracking-[0.3em] text-[#6b6b6b]">
              <span className="text-[#d4ff00]">✦</span>{item}
            </span>
          ))}
        </div>
      </section>
      ...
      <section id="features" className="py-32 px-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {FEATURES.map((feat, i) => <FeatureCard key={i} feature={feat} index={i} />)}
          </div>
        </div>
      </section>
      ...
      <section className="py-40 px-8 relative overflow-hidden">
        <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 80% 60% at 50% 50%, rgba(212,255,0,0.06) 0%, transparent 70%)' }} />
        <div className="absolute inset-0 opacity-[0.03]" style={{ backgroundImage: 'linear-gradient(#d4ff00 1px, transparent 1px), linear-gradient(90deg, #d4ff00 1px, transparent 1px)', backgroundSize: '60px 60px' }} />
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          <RevealText>
            ...
            <h2 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 'clamp(48px, 8vw, 120px)', letterSpacing: '-0.02em', lineHeight: 0.95 }}>
              PRONTO PARA<br />OPERAR NO<br />
              <span style={{ color: '#d4ff00' }}>PRÓXIMO NÍVEL</span><span style={{ color: '#f5f5f0' }}>?</span>
            </h2>
      ...
      <footer className="border-t border-[#1a1a1a] px-8 py-12">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="text-[11px] text-[#6b6b6b] font-mono">© 2025 Animalz Group. Todos os direitos reservados.</div>
          <div className="flex gap-6">
            {['Termos', 'Privacidade', 'Contato'].map(l => (
              <a key={l} href="#" className="text-[11px] text-[#6b6b6b] hover:text-[#d4ff00] transition-colors font-mono tracking-wider">{l.toUpperCase()}</a>
            ))}
          </div>
        </div>
      </footer>
  ...
  return (
    <div ref={ref}
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <span className="text-[9px] font-mono tracking-widest text-[#d4ff00] border border-[#d4ff00]/20 px-2 py-1 rounded-sm">{feature.tag}</span>
      </div>
      <h3 className="text-base font-semibold text-[#f5f5f0] mb-2 group-hover:text-[#d4ff00] transition-colors">{feature.title}</h3>
...
function StatementSection() {
  ...
  return (
    <section className="py-32 px-8 border-y border-[#1a1a1a] relative overflow-hidden">
      <div className="absolute inset-0" style={{ background: 'radial-gradient(ellipse 60% 80% at 30% 50%, rgba(212,255,0,0.03) 0%, transparent 70%)' }} />
      <div ref={ref} className="max-w-6xl mx-auto transition-all duration-1000"
        ...
        <p style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 'clamp(32px, 5vw, 72px)', letterSpacing: '-0.01em', lineHeight: 1.1, color: '#f5f5f0' }}>
        ...
        <div className="mt-10 flex items-center gap-4">
          ...
          <span className="text-[11px] font-mono tracking-widest text-[#6b6b6b]">ANIMALZ EVENTS — PART OF ANIMALZ GROUP</span>
        </div>
      </div>
    </section>
  )
          ...
          {rows.map((row, i) => (
            <div key={i} className="grid grid-cols-3 border-b border-[#1a1a1a] last:border-b-0 hover:bg-[#d4ff00]/2 transition-colors">
              ...
              <div className="px-6 py-4 text-center border-x border-[#1a1a1a]">
                {row.them ? <span className="text-[#6b6b6b] text-base">✓</span> : <span className="text-[#2a2a2a] text-base">✗</span>}
              </div>
              <div className="px-6 py-4 text-center"><span className="text-[#d4ff00] text-base">✓</span></div>
            </div>
          ))}
        </div>
      </div>
