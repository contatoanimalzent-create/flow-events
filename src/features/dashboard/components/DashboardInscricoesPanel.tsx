...
export function DashboardInscricoesPanel() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setStats({
        total: data.length,
        coalizao: data.filter((registration) => registration.army === 'coalizao').length,
        alianca: data.filter((registration) => registration.army === 'alianca').length,
        comSquad: data.filter((registration) => Boolean(registration.squad)).length,
      })
      setLoading(false)
    }
  ...
  const items = [
    ...
    { label: 'Com squad', value: stats.comSquad, icon: UserCheck, color: 'text-status-success', bg: 'bg-status-success/8' },
  ]
              ...
              <div key={item.label} className={cn('flex flex-col gap-1.5 rounded-sm border border-bg-border p-3', item.bg)}>
                ...
                <div className="text-[10px] text-text-muted">
                  {item.label !== 'Total inscritos'
                    ? `${((item.value / stats.total) * 100).toFixed(0)}% do total`
        ...
        <div className="mt-3 flex items-center gap-2">
          <div className="h-2 flex-1 overflow-hidden rounded-full bg-bg-border">
            <div
              ...
              style={{ width: `${(stats.coalizao / stats.total) * 100}%` }}
            />
          </div>
          <div className="h-2 flex-1 overflow-hidden rounded-full bg-bg-border">
            <div
              className="h-full bg-[#0EA5E9] transition-all"
              style={{ width: `${(stats.alianca / stats.total) * 100}%` }}
            />
          </div>
          <span className="font-mono text-[10px] text-text-muted">
            {stats.coalizao} vs {stats.alianca}
          </span>
        </div>
      ) : null}
    </div>
