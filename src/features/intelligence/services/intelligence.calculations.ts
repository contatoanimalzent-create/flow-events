...
function buildRecommendationFromAlert(eventId: string, eventName: string, code: string, sourceAlertId: string): IntelligenceRecommendation {
  const mapping: Record<string, { title: string; description: string; action_label: string; priority: IntelligenceRecommendationPriority }> = {
    sales_below_forecast: {
      ...
      description: 'A receita realizada está abaixo do forecast e precisa de ação comercial imediata.',
    ...
    margin_critical: {
      ...
      description: 'A margem projetada do evento está crítica e demanda corte de custo ou aumento de receita.',
      action_label: 'Ajustar operação',
      priority: 'high',
    },
    financial_divergence: {
      title: 'Resolver divergências financeiras',
    ...
    payout_pending: {
      ...
      description: 'O repasse do evento está retido, divergente ou sem revisão executiva.',
      action_label: 'Abrir financeiro',
      priority: 'high',
    },
    closure_stuck: {
      title: 'Destravar fechamento operacional',
      description: 'O evento ainda possui pendências para fechamento operacional-financeiro.',
      action_label: 'Fechar pendências',
      priority: 'high',
    },
    batch_near_sellout: {
      title: 'Preparar próximo lote',
      description: 'O lote atual está perto de esgotar e precisa de transicao controlada para não perder venda.',
      action_label: 'Planejar virada',
      priority: 'medium',
    },
    invalid_checkin_rate: {
      title: 'Revisar operação de entrada',
      description: 'A taxa de válidação invalida no check-in está acima do esperado.',
  ...
  const selected = mapping[code] ?? {
    ...
    description: 'O evento possui sinais críticos e precisa de acompanhamento executivo.',
  ...
  const healthScores: IntelligenceEventHealth[] = params.events.map((event) => {
    ...
    if (report) {
      if (report.net_sales < report.projected_revenue * 0.75) {
        pushAlert({
          ...
          description: 'A receita liquida realizada está abaixo do ritmo esperado para o evento.',
      ...
      if (report.projected_margin < 0 || report.projected_margin_percent < 10) {
        pushAlert({
          ...
          description: 'A margem projetada do evento está pressionada e pode comprometer o resultado final.',
      ...
      if (report.reconciliation_divergent_count > 0) {
        pushAlert({
          ...
          description: 'Existem pedidos ou pagamentos sem conciliação correta no evento.',
      ...
      if (['held', 'divergent'].includes(report.payout_status) || (report.payout_status === 'draft' && report.net_sales > 0)) {
        pushAlert({
          ...
          description: 'O repasse do evento ainda não está liberado de forma segura para o organizador.',
          recommendation: 'Revisar retenções, divergências e agenda de repasse.',
      ...
      if (report.closure_pending_count > 0 && ['ongoing', 'finished', 'archived'].includes(event.status)) {
        pushAlert({
          ...
          description: 'O evento ainda possui pendências financeiras ou operacionais para fechamento.',
          recommendation: 'Priorizar as pendências antes do encerramento definitivo.',
    ...
    if (nearSelloutBatches.length > 0) {
      pushAlert({
        ...
        description: `${nearSelloutBatches[0].name} está perto de esgotar e precisa de acompanhamento comercial.`,
        recommendation: 'Preparar virada de lote e comunicar urgencia de compra.',
        metricLabel: 'Lotes críticos',
    ...
    if (lowConversionBatches.length > 0) {
      pushAlert({
        ...
        description: `${lowConversionBatches[0].name} está vendendo abaixo do esperado para o volume disponível.`,
    ...
    if (invalidRate > 0.08) {
      pushAlert({
        ...
        title: 'Taxa alta de check-in inválido',
        description: 'A operação de entrada está registrando válidações invalidas acima do esperado.',
    ...
    if (lowThroughputGates.length > 0) {
      pushAlert({
        ...
        description: `${lowThroughputGates[0].name} está abaixo do throughput esperado para a operação ao vivo.`,
