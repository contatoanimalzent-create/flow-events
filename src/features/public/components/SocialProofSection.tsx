  ...
  return (
    <section className="px-5 py-12 md:px-10 lg:px-16 lg:py-16">
      <div className="mx-auto max-w-7xl overflow-hidden rounded-[2.6rem] border border-[rgba(255,255,255,0.08)] bg-[#0e0c0a] p-8 shadow-[0_18px_55px_rgba(0,0,0,0.5)] md:p-12">
        <div className="pointer-events-none absolute right-[-4rem] top-[-4rem] h-56 w-56 rounded-full bg-[#c49a50]/06 blur-3xl" />
        <PublicReveal>
          <div className="max-w-3xl">
            <div className="text-[11px] uppercase tracking-[0.3em] text-[#c49a50]">{eyebrow}</div>
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-3">
          {metrics.map((metric, index) => (
            <PublicReveal key={metric.label} delayMs={index * 90}>
              <div className="rounded-[1.8rem] border border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.04)] p-6 transition-all duration-300 hover:-translate-y-1 hover:border-[#c49a50]/20">
                <div className="text-[11px] uppercase tracking-[0.26em] text-[#6a6058]">{metric.label}</div>
                <div className="mt-4 font-display text-[2.8rem] font-semibold leading-none tracking-[-0.05em] text-[#c49a50]">
                  {metric.value}
                </div>
