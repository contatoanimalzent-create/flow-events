...
export function DashboardConversionChart({ data }: DashboardConversionChartProps) {
  return (
    <div className="card p-5">
      <div className="mb-4">
        <div className="text-[10px] font-mono uppercase tracking-widest text-text-muted">Conversão por evento</div>
        <div className="mt-1 text-lg font-semibold text-text-primary">Taxa de ocupacao entre capacidade e ingressos vendidos</div>
      </div>
      {data.length === 0 ? (
        <div className="py-16 text-center text-sm text-text-muted">Sem eventos suficientes para gerar a conversão.</div>
      ) : (
