...
import { eventsKeys, eventsQueries } from '@/features/events/services'
import type { EventListFilter, EventViewMode } from '@/features/events/types'
...
export function useEventsList(organizationId?: string) {
  ...
  const eventsQuery = useQuery({
    ...(organizationId ? eventsQueries.list(organizationId) : { queryKey: eventsKeys.list('anonymous'), queryFn: async () => [] }),
