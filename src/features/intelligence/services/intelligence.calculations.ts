...
function buildRecommendationFromAlert(eventId: string, eventName: string, code: string, sourceAlertId: string): IntelligenceRecommendation {
  const mapping: Record<string, { title: string; description: string; action_label: string; priority: IntelligenceRecommendationPriority }> = {
    sales_below_forecast: {
      ...
      description: 'A receita realizada esta abaixo do forecast e precisa de acao comercial imediata.',
      action_label: 'Priorizar growth',
      priority: 'high',
    },
    low_conversion_batch: {
      title: 'Revisar lote e precificacao',
      description: 'Existe lote ativo com baixa conversao e risco de travar a tracao comercial do evento.',
      action_label: 'Revisar lote',
      priority: 'medium',
    },
    margin_critical: {
      title: 'Proteger margem projetada',
      description: 'A margem projetada do evento esta critica e demanda corte de custo ou aumento de receita.',
      action_label: 'Ajustar operacao',
      priority: 'high',
    },
    ...
    batch_near_sellout: {
      title: 'Preparar proximo lote',
      description: 'O lote atual esta perto de esgotar e precisa de transicao controlada para nao perder venda.',
      action_label: 'Planejar virada',
      priority: 'medium',
    },
    invalid_checkin_rate: {
      title: 'Revisar operacao de entrada',
      description: 'A taxa de validacao invalida no check-in esta acima do esperado.',
      action_label: 'Ajustar check-in',
      priority: 'high',
    },
    gate_low_throughput: {
      title: 'Reforcar gate critico',
      description: 'Existe gate com throughput abaixo do esperado para a operacao em tempo real.',
      action_label: 'Realocar equipe',
      priority: 'medium',
    },
    staff_insufficient: {
      title: 'Reforcar staff operacional',
      description: 'A alocacao de staff nao cobre a operacao prevista nas portarias ativas.',
  ...
  const healthScores: IntelligenceEventHealth[] = params.events.map((event) => {
    ...
    if (report) {
      if (report.net_sales < report.projected_revenue * 0.75) {
        pushAlert({
          ...
          recommendation: 'Reforcar campanha e revisar a estrategia comercial.',
      ...
      if (['held', 'divergent'].includes(report.payout_status) || (report.payout_status === 'draft' && report.net_sales > 0)) {
        pushAlert({
          ...
          description: 'O repasse do evento ainda nao esta liberado de forma segura para o organizador.',
    ...
    if (lowConversionBatches.length > 0) {
      pushAlert({
        ...
        type: 'sales',
        severity: 'warning',
        title: 'Lote com baixa conversao',
        description: `${lowConversionBatches[0].name} esta vendendo abaixo do esperado para o volume disponivel.`,
        recommendation: 'Revisar oferta, copy e preco do lote.',
    ...
    if (invalidRate > 0.08) {
      pushAlert({
        ...
        description: 'A operacao de entrada esta registrando validacoes invalidas acima do esperado.',
    ...
    if (lowThroughputGates.length > 0) {
      pushAlert({
        ...
        description: `${lowThroughputGates[0].name} esta abaixo do throughput esperado para a operacao ao vivo.`,
    ...
    if (insufficientStaffGates.length > 0) {
      pushAlert({
        ...
        description: `${insufficientStaffGates[0].name} nao possui cobertura operacional suficiente.`,
        recommendation: 'Realocar staff e cobrir o gate critico imediatamente.',
