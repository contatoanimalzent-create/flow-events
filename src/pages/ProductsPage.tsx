  ...
  return (
    <div className="admin-page">
      <div className="admin-header">
        <div>
          <div className="admin-eyebrow">Comercial e operação</div>
          <h1 className="admin-title">PDV<span className="admin-title-accent">.</span></h1>
          <p className="admin-subtitle">Aqui o time vende. Catálogo comercial, pedido rápido e caixa ficam juntos; saldo, ruptura e reposicao saem da frente e vivem em Estoque.</p>
        </div>
        <div className="grid gap-3 md:grid-cols-2">
          <div className="card p-5"><div className="text-[11px] uppercase tracking-[0.32em] text-[#0057E7]">Uso correto</div><div className="mt-4 font-display text-[2rem] leading-none text-text-primary">Venda e caixa.</div><p className="mt-4 text-sm leading-7 text-text-muted">Monte o catálogo, acelere atendimento e feche pedido sem misturar leitura de estoque.</p></div>
          <div className="card p-5"><div className="text-[11px] uppercase tracking-[0.32em] text-[#0057E7]">Não confundir</div><div className="mt-4 text-sm leading-7 text-text-secondary">PDV vende. Estoque controla saldo e ruptura. Se a dúvida for reposicao, a equipe deve olhar Estoque.</div></div>
        </div>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {[
          ...
          { label: 'Preço medio', value: formatCurrency(averagePrice), tone: 'text-brand-blue' },
      ...
      <div className="admin-filterbar">
        <div className="flex flex-wrap items-center gap-2">{[{ key: 'pdv', label: 'PDV rápido' }, { key: 'catalog', label: 'Catálogo comercial' }].map((item) => <button key={item.key} type="button" onClick={() => setTab(item.key as typeof tab)} className={cn('rounded-full px-4 py-2 text-xs font-medium transition-all', tab === item.key ? 'bg-brand-primary text-white' : 'border border-bg-border text-text-secondary hover:text-text-primary')}>{item.label}</button>)}</div>
      ...
      <ActionConfirmationDialog open={Boolean(pendingDeleteProduct)} title="Excluir item comercial" description={pendingDeleteProduct ? `O item ${pendingDeleteProduct.name} deixara de aparecer no catálogo comercial e no PDV.` : undefined} impact="Historicos antigos podem perder a referência visual deste item e a equipe precisara cadastrar novamente para voltar a vender." confirmLabel="Excluir item" onCancel={() => setPendingDeleteProduct(null)} onConfirm={async () => { if (!pendingDeleteProduct) return; await handleDelete(pendingDeleteProduct.id); setPendingDeleteProduct(null) }} />
    </div>
  )
}
...
function ProductFormModal({ organizationId, events, product, defaultEventId, onClose, onSaved }: { organizationId: string; events: EventItem[]; product: Product | null; defaultEventId: string; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[rgba(7,6,7,0.76)] p-4 backdrop-blur-md"><div className="w-full max-w-3xl overflow-hidden rounded-[2rem] border border-white/8 bg-[#12100f] shadow-[0_30px_120px_rgba(0,0,0,0.62)]"><div className="flex items-start justify-between border-b border-white/6 px-6 py-5"><div><div className="text-[10px] uppercase tracking-[0.34em] text-[#0057E7]">Catálogo comercial</div><div className="mt-3 font-display text-[2.3rem] leading-none tracking-[-0.04em] text-text-primary">{product ? 'Editar item.' : 'Novo item.'}</div><p className="mt-3 text-sm leading-6 text-text-muted">Cadastre o item para venda. Saldo e threshold existem para apoio, mas a leitura operacional profunda sai para Estoque.</p></div><button type="button" onClick={onClose} className="btn-secondary h-10 w-10 rounded-full px-0"><X className="h-4 w-4" /></button></div><div className="grid gap-6 p-6 lg:grid-cols-[minmax(0,1fr)_280px]"><div className="space-y-4"><div className="grid gap-4 md:grid-cols-2"><div className="md:col-span-2"><label className="input-label">Nome do item</label><input className="input" value={form.name} onChange={(event) => setField('name', event.target.value)} placeholder="Ex: Combo premium, agua, camiseta" /></div><div><label className="input-label">SKU</label><input className="input" value={form.sku} onChange={(event) => setField('sku', event.target.value)} placeholder="Ex: PDV-001" /></div><div><label className="input-label">Categoria</label><select className="input" value={form.category} onChange={(event) => setField('category', event.target.value as ProductCategory)}>{Object.entries(CATEGORY_CONFIG).map(([key, config]) => <option key={key} value={key}>{config.label}</option>)}</select></div></div>{events.length ? <div><label className="input-label">Evento vinculado</label><select className="input" value={eventId} onChange={(event) => setEventId(event.target.value)}><option value="">Catálogo geral</option>{events.map((event) => <option key={event.id} value={event.id}>{event.name}</option>)}</select></div> : null}<div className="grid gap-4 md:grid-cols-2"><div><label className="input-label">Preço de venda</label><input className="input" type="number" min={0} step={0.01} value={form.price} onChange={(event) => setField('price', event.target.value)} /></div><div><label className="input-label">Preço de custo</label><input className="input" type="number" min={0} step={0.01} value={form.cost_price} onChange={(event) => setField('cost_price', event.target.value)} /></div></div><div className="grid gap-4 md:grid-cols-2"><div><label className="input-label">Saldo inicial</label><input className="input" type="number" min={0} value={form.stock_quantity} onChange={(event) => setField('stock_quantity', event.target.value)} /></div><div><label className="input-label">Threshold de alerta</label><input className="input" type="number" min={0} value={form.stock_alert_threshold} onChange={(event) => setField('stock_alert_threshold', event.target.value)} /></div></div><div><label className="input-label">Descrição</label><textarea className="input min-h-[120px] resize-y" value={form.description} onChange={(event) => setField('description', event.target.value)} /></div></div><div className="space-y-4"><div className="card p-5"><div className="text-[11px] uppercase tracking-[0.32em] text-[#0057E7]">Checklist rápido</div><div className="mt-4 space-y-3 text-sm leading-6 text-text-secondary"><div>1. Nomeie o item de forma clara para o operador.</div><div>2. Defina o preço comercial.</div><div>3. Ative apenas se puder vender agora.</div></div></div><div className="card p-5"><div className="flex items-center justify-between"><span className="text-sm font-medium text-text-primary">Disponível no PDV</span><button type="button" onClick={() => setIsActive((current) => !current)} className={cn('relative h-7 w-14 rounded-full transition-all', isActive ? 'bg-brand-acid' : 'bg-white/10')}><span className={cn('absolute top-1 h-5 w-5 rounded-full bg-white transition-all', isActive ? 'left-8' : 'left-1')} /></button></div><div className="mt-3 text-xs leading-6 text-text-muted">Ativar coloca o item no PDV. Pausar remove do caixa sem apagar o cadastro.</div></div>{error ? <div className="rounded-[1.4rem] border border-status-error/20 bg-status-error/10 px-4 py-3 text-sm text-status-error">{error}</div> : null}</div></div><div className="flex items-center justify-between border-t border-white/6 px-6 py-5"><button type="button" onClick={onClose} className="btn-secondary">Cancelar</button><button type="button" onClick={handleSave} disabled={saving} className="btn-primary min-w-[160px] justify-center">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : null}{saving ? 'Salvando...' : product ? 'Salvar item' : 'Criar item'}</button></div></div></div>
}
